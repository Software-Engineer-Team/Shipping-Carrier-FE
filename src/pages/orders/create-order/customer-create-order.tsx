...
import { useApiUrl, useCustom, useCustomMutation, useGetIdentity, useNavigation, useTranslate } from '@refinedev/core';
...
import { IAddressResponse } from '../../../components/form/select-address';
import { USER_ID_KEY } from '../../../constants';
import { ICarrier, IUser } from '../../../interfaces/types';
import { APIEndPoint, StringUtils, initialFormCreateOrder } from '../../../utils';
...
export interface CarrierShipmentFeeResponse {
  carrier: ICarrier;
...
export const CustomerCreateOrder: React.FC = () => {
  ...
  const { data: dataAddresses, isSuccess: isSuccessAddresses } = useCustom<IAddressResponse>({
    config: {
      query: {
  ...
  }, [dataAddresses?.data?.data, isSuccessAddresses]);

  useEffect(() => {
    setTotalFee(0);
    if (watchFromProvinceCode && watchToProvinceCode && watchWeight) {
      ...
      mutate(
        {
          ...
          method: 'post',
          url: `${apiUrl}${APIEndPoint.SHIPMENT_FEE}/${userData?.id}`,
          values: {
  ...
  }, [watchHasInsuranceFee, watchParcelValue, watchWeight, watchFromProvinceCode, watchToProvinceCode, watchHeight, watchLength, watchWidth, mutate, apiUrl, userData?.id]);
  ...
  const setFromUserInfo = (data: any) => {
  ...
  return (
    <Create title={translate('orders.titles.create')} saveButtonProps={saveButtonProps} isLoading={formLoading}>
      <Form form={form} {...formProps} layout="vertical" labelAlign="left" size={'large'} onFinish={handleSubmit} labelWrap initialValues={initialFormCreateOrder}>
        <Row gutter={[16, 16]}>
          <Col xs={24} xl={12}>
            ...
            >
              <Input allowClear prefix={<DropboxOutlined />} placeholder={translate('orders.placeholders.productName')} />
            </Form.Item>
            <Form.Item
          ...
          <Col xs={24} xl={12}>
            ...
            <Form.Item label={translate('orders.labels.cod')} name="cash_on_delivery">
              <InputNumber placeholder={translate('orders.placeholders.cod')} formatter={value => StringUtils.formatterCurrency(value)} min={0} max={20000000} style={{ width: '100%' }} />
            </Form.Item>
            ...
            <Form.Item label={translate('orders.labels.deliveryInstructions')} name="delivery_instructions">
              <Select allowClear placeholder={translate('orders.placeholders.deliveryInstructions')} mode="tags" style={{ width: '100%' }} tokenSeparators={[',']} options={instructionOptions} />
            </Form.Item>
            <Divider orientation="left" orientationMargin="0">
